...
import {
  getAppPushStatus,
  registerForAppPush,
} from "../../lib/appPushNotifications";

const OVERLAY_DISMISS_KEYS = [
  "hooks.push.overlay.dismissed_at",
  "hooks.news_push.overlay.dismissed_at",
];
...
const readOverlayDismissedAt = () => {
  ...
  for (const key of OVERLAY_DISMISS_KEYS) {
...
const writeOverlayDismissedAt = (value) => {
  ...
  window.localStorage.setItem(OVERLAY_DISMISS_KEYS[0], String(value));
  OVERLAY_DISMISS_KEYS.slice(1).forEach((key) =>
...
const AppPushOptInPrompt = () => {
  ...
  useEffect(() => {
    ...
    const syncStatus = async () => {
      const nextStatus = await getAppPushStatus();
      if (!active) return;
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      setFeedback(error?.message || "Unable to enable notifications right now.");
      await refreshStatus();
    } finally {
      setBusy(false);
    }
