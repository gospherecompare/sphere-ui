...
import { FaArrowRight, FaExchangeAlt, FaFire, FaMobileAlt } from "react-icons/fa";
...
const RecommendedSmartphones = () => {
  ...
  const rows = useMemo(() => {
    ...
    const phones = catalog.filter((product) => product.type === "smartphones").slice(0, 8);
    return phones.slice(0, 6).map((product, index) => {
  ...
  return (
    <section className="home-v2-section home-v2-compare" aria-labelledby="home-v2-compare-title">
      <div className="hooks-container">
        <HomeSectionHeader
          eyebrow="Comparison arena"
          titleId="home-v2-compare-title"
          title="See the decision, not just two spec sheets"
          copy="Popular real-world matchups from the Hooks comparison feed, arranged as one focused arena with supporting battles."
        ...
        {lead ? (
          <div className="home-v2-compare-arena">
            <Link
              to={buildCanonicalComparePath({ leftName: lead.leftName, rightName: lead.rightName })}
            ...
            >
              ...
              <div className="home-v2-compare-main__devices">
                ...
                <div className="home-v2-compare-device">
                  <DevicePortrait image={lead.rightImage} name={lead.rightName} />
                  <h3>{lead.rightName}</h3>
                </div>
              </div>
              <div className="home-v2-compare-main__footer">
                <span><b>{formatCompact(lead.count)}</b><small>comparison actions</small></span>
                <strong>Open full comparison <FaArrowRight aria-hidden="true" /></strong>
              </div>
            </Link>
            ...
            <div className="home-v2-compare-list">
              {rest.map((comparison, index) => (
                ...
                >
                  <span className="home-v2-compare-list__rank">0{index + 2}</span>
                  <div>
                    <p>{comparison.leftName}</p>
                    <span>versus</span>
                    <p>{comparison.rightName}</p>
                  </div>
                  <small>{comparison.count ? `${formatCompact(comparison.count)} compares` : "Compare now"}</small>
                  <FaArrowRight aria-hidden="true" />
                </Link>
              ))}
              <Link to="/compare" className="home-v2-compare-list__create">
                <span><FaExchangeAlt aria-hidden="true" /></span>
                <div><b>Create your own matchup</b><small>Choose any two published products</small></div>
                <FaArrowRight aria-hidden="true" />
              </Link>
            </div>
          </div>
