...
const normalizeSmartphoneCollection = (body, preferredKeys = []) => {
  ...
  return (arr || []).map((d) => {
    ...
    return {
      ...
      published:
        publishMap[id] != null
          ? !!publishMap[id].published
          : Boolean(d.published ?? d.is_published ?? d.publish ?? false),
      ...
      launch_date: normalizeDate(d.launch_date),
      created_at: normalizeDate(d.created_at ?? d.createdAt ?? null),
